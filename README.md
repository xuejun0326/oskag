...
oskag 对操作系统内核仓库做了专门适配，会按启动、内存、调度、文件系统等固定维度展开分析。内核家族识别覆盖五类技术路线：ArceOS-Starry、rCore-Rust（rCore 教学系及 Rust 自研内核）、xv6-C（xv6 教学血缘的 C 实现）、Unix-C（Unix-like C 自研内核），以及兜底的 unknown。面对其他类型的项目（编译器、库、应用等），则根据目录结构自动组织章节，并由大模型推断其项目类型。

## 提交文档 [](https://gitlab.eduxiji.net/T2026106989911008/project3136859-388690/-/blob/main/%E6%8F%90%E4%BA%A4%E6%96%87%E6%A1%A3%E6%9C%80%E7%BB%88%E7%89%88.md)

## 汇报ppt [](https://gitlab.eduxiji.net/T2026106989911008/project3136859-388690/-/blob/main/proj18-%E7%B2%BE%E5%8B%A4%E6%B1%82%E5%AD%A6-%E5%88%9D%E8%B5%9B%E6%B1%87%E6%8A%A5.pptx)

## 视频演示 https://pan.baidu.com/s/1brOtKTiKgmXv2RW2k2jB3g?pwd=u3a5   提取码u3a5
...
└── oskag/                   Python 包源码
    ...
    ├── facts/               确定性事实抽取
    │   ├── profiles.py      内核家族画像（五类家族探针 + 自由模式兜底）
    │   ├── project_type.py  项目类型识别
    │   ├── metadata.py      解析仓库 metadata.md（参赛年份 / 学校 / 队伍）
    │   ├── scan.py          文件扫描与代码行统计
    │   ├── cargo.py         Cargo 依赖
    │   ├── syscalls.py      syscall 表抽取（Rust / C 双路径）
    │   ├── signatures.py    函数签名
    ...
    │   ├── _modules.py      模块发现与命名（自由模式主导目录下钻）
...
仓库地图（RepoMap）的实现参考了 [aider](https://github.com/Aider-AI/aider)，agent 循环的设计参考了 [gptme](https://github.com/ErikBjare/gptme)。内核基座相关项目包括 oscomp/arceos、oscomp/starry-next、rCore-Tutorial-v3 与 mit-pdos/xv6-riscv。
